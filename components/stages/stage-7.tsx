...
export default function Stage7() {
  ...
  const pending = records.filter((r) => r.stage === 7 && r.status === "pending");
  ...
  const commonValid = form.invoiceNumber && form.srnNumber && form.qcRequirement;
  const independentValid =
    form.receivedQty && form.invoiceNumber && form.invoiceDate && form.srnNumber && form.qcRequirement;
            ...
            <Select value="" onValueChange={() => {}}>
              ...
              <SelectContent className="w-64">
                <div className="p-2">
                  ...
                  {(activeTab === "pending" ? PENDING_COLUMNS : HISTORY_COLUMNS).map((col) => (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {PENDING_COLUMNS.filter((c) => selectedPendingColumns.includes(c.key)).map(
                      (c) => (
                  ...
                  {pending.map((rec) => {
                    ...
                    return (
                      <TableRow key={rec.id}>
                            ...
                            if (col.key === "deliveryDate")
                              return (
                                <TableCell key={col.key}>
                                  {rec.data[col.key]
                                    ? new Date(rec.data[col.key] as string).toLocaleDateString("en-IN")
                                    : "-"}
                                </TableCell>
                              );
                        ...
                        <TableCell>
                          <Button variant="outline" size="sm" onClick={() => openModal(rec.id)}>
                            Record Receipt
                          </Button>
                        </TableCell>
                      </TableRow>
              ...
              <Table>
                <TableHeader className="bg-gray-100 sticky top-0">
                  <TableRow className="border-b-2">
                    {HISTORY_COLUMNS.filter((c) => selectedHistoryColumns.includes(c.key)).map(
                      (c) => (
                        <TableHead key={c.key} className="font-semibold">{c.label}</TableHead>
                      )
                    )}
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {completed.map((record) => {
                    ...
                    const stage7History = record.history.find((h) => h.stage === 7);
                    const historyData = stage7History ? stage7History.data : record.data;
                    const lift = (historyData.liftingData as LiftingEntry[] ?? [])[0] ?? {};
                    const vendor = getVendorData({ ...record, data: historyData });
                    ...
                    return (
                      <TableRow key={record.id} className="bg-green-50">
                        {HISTORY_COLUMNS.filter((c) => selectedHistoryColumns.includes(c.key)).map(
                          (col) => {
                            // Handle date fields
                            if (col.key === "deliveryDate" || col.key === "invoiceDate") {
                              return (
                                <TableCell key={col.key}>
                                  {historyData[col.key]
                                    ? new Date(historyData[col.key] as string).toLocaleDateString("en-IN")
                                    : "-"}
                                </TableCell>
                              );
                            }
                            ...
                            if (col.key === "receivedItemImage" || col.key === "poCopy" || col.key === "vendorAttachment") {
                            ...
                            const liftValue =
                              col.key === "liftNumber"
                                ...
                                ? `₹${lift.advanceAmount || "-"}`
                                : col.key === "paymentDate"
                                ? lift.paymentDate ? new Date(lift.paymentDate).toLocaleDateString("en-IN") : "-"
                                : undefined;

                            if (liftValue !== undefined) {
                              return <TableCell key={col.key}>{liftValue}</TableCell>;
                            }
                            ...
                            if (vendorValue !== undefined) {
                              return <TableCell key={col.key}>{vendorValue}</TableCell>;
                            }

                            // Handle payment amounts
                            if (col.key === "paymentAmountHydra" || col.key === "paymentAmountLabour" || col.key === "paymentAmountHemali") {
                              return (
                                <TableCell key={col.key}>
                                  {historyData[col.key] ? `₹${historyData[col.key]}` : "-"}
                                </TableCell>
                              );
                            }

                            // Default: show from historyData
                            return <TableCell key={col.key}>{String(historyData[col.key] ?? "-")}</TableCell>;
                          }
                        )}
                      </TableRow>
                    );
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto space-y-6 pr-2">
            ...
            <div className="space-y-2">
              <Label>Invoice Type <span className="text-red-500">*</span></Label>
              <Select
                value={invoiceType}
                onValueChange={(v) => {
            ...
            <div className="grid grid-cols-2 gap-4">
              {invoiceType === "independent" && (
                <>
                  <div className="space-y-2">
                    <Label>Lift #</Label>
                    <Input value={form.liftNumber} readOnly className="bg-gray-50" />
                  </div>
                  <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label>SRN # <span className="text-red-500">*</span></Label>
                <Input
                  value={form.srnNumber}
                  onChange={(e) => setForm({ ...form, srnNumber: e.target.value })}
                  required
                />
              </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                accept=".jpg,.jpeg,.png"
                onChange={(e) =>
                  setForm({ ...form, receivedItemImage: e.target.files?.[0] ?? null })
                }
                className="hidden"
              />
              <label
              ...
              {form.receivedItemImage && (
                <div className="mt-2 p-2 bg-gray-50 border rounded flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-gray-700">{form.receivedItemImage.name}</span>
                  </div>
                  <button
                    type="button"
            ...
            {invoiceType === "independent" && (
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, billAttachment: e.target.files?.[0] ?? null })
                  }
                  className="hidden"
                />
                <label
                ...
                {form.billAttachment && (
                  <div className="mt-2 p-2 bg-gray-50 border rounded flex items-center justify-between">
                    <div className="flex items-center">
                      <FileText className="w-4 h-4 text-gray-500 mr-2" />
                      <span className="text-sm text-gray-700">{form.billAttachment.name}</span>
                    </div>
                    <button
                      type="button"
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm({ ...form, paymentAmountHydra: e.target.value })}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm({ ...form, paymentAmountLabour: e.target.value })}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm({ ...form, paymentAmountHemali: e.target.value })}
                  />
                </div>
              </div>
            </div>
